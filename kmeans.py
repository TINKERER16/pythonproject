...
def select_centriod(no_of_clusters):#gettting random centriod for first iteration#
    global centriod1
    a = -1
...
def cluster(centriod1):#does the clustering part#
    ...
    for i in datasets:#getting which cluster is best#
        if i not in centriod1:
            ...
            a.append(i)
            minimum=-1
    for element in clusters:#means who are not part of dataset are removed#
...
while clusters1!=clusters2:
    ...
    if clusters1!=clusters2:#if not equal making  2 as first one and clearing 2 one#
        clusters1=clusters2
        centriod1=[]
        for i in range(no_of_clusters):
